...
docker run -d \
  ...
  icekale/stock-lu-tracker:v0.1.23
```
...
默认示例已固定到 `v0.1.23`，这样部署更稳定；如果你希望始终跟随最新构建，可手动改回 `latest`。
...
```bash
docker build -t icekale/stock-lu-tracker:v0.1.23 .
docker run -d --name stock-lu -p 8787:8787 -v $(pwd)/data:/app/data -e ADMIN_PASSWORD='请改成你的后台密码' -e ADMIN_COOKIE_SECURE=false icekale/stock-lu-tracker:v0.1.23
```
...
```bash
./scripts/docker-publish-multiarch.sh v0.1.23
```
...
services:
  stock-lu:
    image: icekale/stock-lu-tracker:v0.1.23
...
2. 将 compose 或模板里的镜像标签改成新版本，例如：`icekale/stock-lu-tracker:v0.1.23`
